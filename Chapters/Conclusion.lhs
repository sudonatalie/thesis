...
Our projection inlining optimisation alone yields a 10-20\% improvement in
execution time in our tests. The additional optimisations for case squashing,
pattern let generationg, and pattern let floating within functions do not yet
result in measurably significant benefits in memory usage or run time, however,
they are important transformations in preparation for cross-call floating. Our
cross-call floating optimisation is currently in development, and we expect
that the sharing benefit generated by its implementation will have the greatest
impact.
