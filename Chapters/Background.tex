...
Agda is a dependently typed functional programming language. We assume the reader has seen a functional language before, such as Haskell. In traditional functional languages, types may depend on other types. For example, the type signature \lstinline{xs :: Vector a} in Haskell denotes a vector containing elements of type \lstinline{a}, where \lstinline{a} is a type variable. In a dependently typed language like Agda, types can depend not only on other types, but also on values. Consider the \AgdaFunction{replicate} in Figure~\ref{code:replicate}, which produces a \AgdaDatatype{Vec}tor of elements of type \AgdaBound{A} with length \AgdaBound{n}.\cite{norell2009}
